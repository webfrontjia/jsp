...
public class CheckCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		drawRandomLine(g);
		//6.写在图片上随机数
		String random = drawRandomNum((Graphics2D) g,ran);//根据客户端传递的ran标识生成验证码图片
		//7.将随机数存在session中
	 ...
	 *
	 */
	private String drawRandomNum(Graphics2D g,String ran) {
		// 设置颜色
		...
		if ( null != ran) {
			...
			}else if (ran.equals("nl")) {
				// 截取数字和字母的组合
				return createRandomChar(g, baseNumLetter);
			}else if (ran.equals("n")) {
				// 截取数字
